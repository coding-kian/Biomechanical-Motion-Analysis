...
def frame_extraction(frame_dir, video_path, downscale):
...
def load_frames(frame_dir) -> None:
    sorted(os.path.join(frame_dir, f) for f in os.listdir(frame_dir) if f.lower().endswith((".png",".jpg",".jpeg")))
    
if __name__ == "__main__":
    TARGET_FPS = 12
    downscale = 0.25 # 1.0 = original size, 0.5 = half resolution
    quality = 75 # quality: 0–100 (higher = better, bigger)
    FRAME_DIR = r"C:\Users\super\Desktop\Professional Coder\video analysis\frames"
    frame_extraction(FRAME_DIR, video_path="flip.mp4", downscale=downscale, quality=quality)
